...
namespace bd_rgr
{
    ...
    public abstract class BaseModel : DbContext
    {
        ...
        {
            optionsBuilder.UseNpgsql(@"Host=localhost;Database=lab1;Username=postgres;Password=1111");
        }

        public abstract UInt16 GetEnum(string name);
        public abstract Dictionary<string, object> GetDict(BaseContext context);
        public List<Dictionary<string, object>> GetDict(List<BaseContext> context)
        {
            var list = new List<Dictionary<string, object>>();
            foreach (var cont in context)
            {
                list.Add(GetDict(cont));
            }

            return list;
        }
        public abstract BaseContext? Find(UInt16 column, object value);
        public abstract List<BaseContext> Find();
        public abstract void Add(Dictionary<string, object> values);
        public abstract void Add(List<Dictionary<string, object>> values);
        public abstract void Remove(UInt16 column, object value, int count = 1);
        public abstract void Remove(UInt16 column, object value, bool greater);
        public abstract void Edit(UInt16 column, object value, Dictionary<UInt16, object> chVal, bool one = false);
        //
